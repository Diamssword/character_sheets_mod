...
public class ScrollTooltipPositioner implements TooltipPositioner {
	...
	private final ClothButtonComponent widget;

	public ScrollTooltipPositioner(ClothButtonComponent widget) {
		this.widget = widget;
	}
	...
	public Vector2ic getPosition(int screenWidth, int screenHeight, int x, int y, int width, int height) {
		...
		if (j + i <= k) {
			...
			vector2i.y = vector2i.y - (i + getOffsetY(vector2i.y, (int) (this.widget.getY() + this.widget.getHeight()+widget.parent.getScrollY()), this.widget.getHeight()));
		}
