package com.diamssword.characters.api.appearence;

import net.minecraft.nbt.NbtCompound;
import net.minecraft.util.Identifier;

public record Cloth(Identifier id, String name, LayerDef layer, String collection,String level) {
		public NbtCompound toNBT() {
			var res = new NbtCompound();
			res.putString("name", name);
			res.putString("layer", layer.getId());
			res.putString("collection", collection);
			res.putString("level", level);
			return res;
		}

		public static Cloth fromNBT(NbtCompound comp, Identifier id,LayerDef layer) {
			try {

				return new Cloth(id, comp.getString("name"), layer, comp.getString("collection"),comp.getString("level"));
			} catch (IllegalArgumentException e) {
				return null;
			}
		}
	}